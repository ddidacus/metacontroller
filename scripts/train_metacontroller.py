...
def _collect_one_successful(env_config, env_mode, num_steps, state_shape, max_seed, min_trajectory_len=10):
    ...
    while True:
        ...
        if traj is not None and traj["length"] >= min_trajectory_len:
            env.close()
            return traj


def collect_batch(env_config, env_mode, batch_size, num_steps, state_shape, max_seed, num_workers=1, min_trajectory_len=10):
    from concurrent.futures import ProcessPoolExecutor

    args = (env_config, env_mode, num_steps, state_shape, max_seed, min_trajectory_len)
...
def initialize_weights_eye(module):
    if isinstance(module, nn.Linear):
...
TRANSFORMER_INIT_FN = initialize_weights_xavier
METACONTROLLER_INIT_FN = initialize_weights_eye
...
def train(
    ...
    discovery_entropy_warmup_steps = 0,
    ...
    if use_wandb:
        wandb_config = {
            ...
            "discovery_entropy_loss_weight": discovery_entropy_loss_weight
        }
    ...
    def store_checkpoint(step: int = None, is_discovering: bool = False):
        if accelerator.is_main_process:
            ...
            ckpt = {
                ...
                "gradient_step": (wandb_step_offset + step) if exists(step) else 0,
    ...
    if exists(load_transformer_weights_path):
        ...
        for name, child in model.named_children():
            if name != "meta_controller":
                child.apply(TRANSFORMER_INIT_FN)
        accelerator.print("Initialized transformer")
    ...
    if exists(load_meta_controller_weights_path):
        ...
        meta_controller.apply(METACONTROLLER_INIT_FN)
        accelerator.print("Initialized meta_controller")
    ...
    if existing_ckpts:
        ...
        if step_numbers:
            ...
            if restart_discovery:
                # find the last BC checkpoint to load clean transformer weights
                ckpt = None
                for _, path in reversed(step_numbers):
                    candidate = torch.load(path, map_location="cpu", weights_only=False)
                    if not candidate.get("is_discovering", False):
                        ckpt = candidate
                        accelerator.print(f"--restart_discovery: loading BC checkpoint {path}")
                        break
                if ckpt is None:
                    accelerator.print("--restart_discovery: no BC checkpoint found, falling back to latest")
    ...
    skip_bc = load_transformer_weights_path is not None or restart_discovery
    ...
    wandb_step_offset = 0
    if restart_discovery:
        resume_step = discovery_phase_start
        wandb_step_offset = ckpt.get("gradient_step", 0) if existing_ckpts else 0
        accelerator.print(f"--restart_discovery: jumping to discovery phase at step {resume_step}, wandb offset={wandb_step_offset}")
    ...
    for gradient_step in progress_bar:
        ...
        with accelerator.accumulate(model):
            ...
            metacontroller_warmup_coeff = 1.0
            if is_discovering and metacontroller_warmup_steps > 0:
                metacontroller_warmup_coeff = min(1.0, discovery_step / metacontroller_warmup_steps)
            ...
            if modality == MODALITY_RESNET_RGB:
                (losses, meta_controller_output), visual_autoencoder_loss = model(
                    ...
                    control_signal_multiplier = metacontroller_warmup_coeff,
                )
            else:
            ...
            if is_behavior_cloning:
                ...
                print(gradient_step)
                entropy_weight = discovery_entropy_loss_weight if discovery_step >= discovery_entropy_warmup_steps else 0.0
                ...
                loss = (
                    ...
                    + kl_loss * metacontroller_warmup_coeff
                    + ratio_loss * discovery_ratio_loss_weight * metacontroller_warmup_coeff
                )
                ...
                log = dict(
                    ...
                    metacontroller_warmup_coeff = metacontroller_warmup_coeff,
                )
                if exists(visual_autoencoder_loss):
        ...
        if gradient_accumulation_steps is None or gradient_step % gradient_accumulation_steps == 0:
            ...
            accelerator.log(log_dict, step=wandb_step_offset + gradient_step)
        ...
        if (gradient_step + 1) % eval_steps == 0 and is_behavior_cloning and accelerator.is_main_process:
            ...
            with torch.no_grad():
                ...
                accelerator.log(bc_eval_log, step=wandb_step_offset + gradient_step)
        ...
        if (gradient_step + 1) % eval_steps == 0 and is_discovering:
            if use_wandb and accelerator.is_main_process:
                visualize_switch_betas(
                    ...
                    episode_lens = episode_lens,
                    gradient_step = wandb_step_offset + gradient_step + 1,
                    num_samples = 3,
                    accelerator = accelerator
                )
