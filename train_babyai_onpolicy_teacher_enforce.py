...
from babyai_env import (
    create_env as _babyai_create_env,
    get_missions_embeddings,
)

def is_new_suite_env(env_id):
    NEW_SUITE_ENVS = ["NGoals"]
    return env_id in NEW_SUITE_ENVS

def create_env(env_id, **kwargs):
    if is_new_suite_env(env_id):
        from environments.ngoals import NGoalsEnv
        from minigrid.wrappers import SymbolicObsWrapper
        import string
        from babyai_env import BabyAISharedMemoryWrapper
...
def main(
    ...
    if exists(npy_seedfile) and (isinstance(npy_seedfile, (str, bytes, Path)) or hasattr(npy_seedfile, '__fspath__')):
        ...
        if isinstance(seed, int):
            ...
            group_seeds = [int(x.strip()) for x in seed.split(',') if x.strip()]
        else:
            group_seeds = list(seed)
        assert len(group_seeds) >= 1, "seed list must not be empty"
